...
##########################################
# training the model
test_losses, losses, disp_val_losses, best_test_loss = train(
    spt_dataset, spc_dataset, device, stats_list, args
)
...
model_name = (
    "model_spectralGNN_w_X_StressType_nl"
    + str(args.num_layers)
    + "_bs"
    + str(args.batch_size)
    + "_hd"
...
model = SpectralMeshGraphNet(
    num_node_features,
    num_edge_features,
    num_sp_node_features,
    num_sp_edge_features,
    args.hidden_dim,
    num_ext_force,
    num_classes,
    args,
...
eval_data_loader = visualize(
    dataset_vis, spc_test_dataset, model, anim_path, args, stats_list, skip=1
)
